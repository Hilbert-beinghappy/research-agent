...
Canonical writes and backups acquire one project writer lease containing PID, host, nonce, and expiry. A renewable lease serializes independent processes; stale same-host owners can be reclaimed only after their process has ended. Writes still use expected manifest and record revisions. A multi-file transaction stages content, verifies old/new hashes, writes records, and updates the manifest last. On non-Windows platforms, it syncs parent directories after rename and fails the operation if that sync fails. Windows currently performs no parent-directory sync, so power-loss durability of directory entries on Windows is not qualified. Interrupted work remains under `.research/transactions/pending`; `/research-recover` requires an interactive choice to commit or roll it back. `/research-validate` reports mixed state, broken hashes, references, paths, or records rather than repairing them silently.
