...
Run the bounded scaling harness with `npm run diagnose:transactions -w packages/research-agent -- --counts 100,250,500 --processes 1,2 --repetitions 1 --max-elapsed-ms 225000`. The manual `Research Agent Transaction Diagnostics` workflow applies the same harness to Ubuntu, a fixed macOS image, and `macos-latest`, with a ten-minute job limit, a 240-second watchdog snapshot, and an explicit 225-second per-case budget. Final macOS qualification uses `--counts 500 --processes 2 --repetitions 3`. Diagnostic completion does not qualify a release and does not replace the unchanged 300-second, 1,000-write deterministic test or its consistency assertions.
