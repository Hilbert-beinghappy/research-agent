...
v1.0 release qualification uses Pi npm packages 0.83.0 and 0.84.0. The authoritative tested matrix is `docs/compatibility/pi-baselines.json`; an unlisted Pi main commit is not implicitly supported.
...
4. Open the v0.1–v0.5 project. It remains read-only and reports `migration_required`.
...
Migration adds missing record-set declarations and directories, advances the manifest once, and leaves every historical canonical record byte-for-byte unchanged. It is idempotent after success.
...
- A project already carrying the v1.0 manifest but a pending journal completes the journal move on retry.
