...
An Adapter contribution needs a capability snapshot, recorded contract tests, failure semantics, credential and policy documentation, fixture provenance, and a distributable license. The v1.0 in-process source contract remains experimental and does not confer sandbox isolation. Governance and the RFC process are documented in `GOVERNANCE.md` and `docs/rfcs/0000-template.md`.
