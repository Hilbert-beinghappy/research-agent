...
const designKinds = new Set(["research_question_version", "concept", "theory_relation", "design_decision", "protocol"]);
...
describe("v0.1 to v0.2 project migration", () => {
	it("prepares without mutation, resumes the pending commit, and rolls back an unchanged project", async () => {
		...
		expect(migrated.manifest.recordSets.filter(({ kind }) => designKinds.has(kind)).map(({ kind }) => kind)).toEqual([
			...designKinds,
		]);
	...
	it("finishes a rollback after the v0.1 manifest was already restored", async () => {
		const migrationId = await prepareProjectMigration(projectRoot);
