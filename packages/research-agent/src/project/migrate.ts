...
export function buildV0_4Manifest(raw: JsonValue): ResearchProjectManifest {
	...
	if (
		...
		throw new TypeError("Project is not a valid v0.3 migration source");
	}
...
export async function prepareProjectMigration(projectRoot: string): Promise<string> {
	...
	const targetManifest = buildV0_4Manifest(raw);
...
export async function rollbackProjectMigration(projectRoot: string, migrationId: string): Promise<OpenedProject> {
	...
	if (currentHash !== journal.newManifestHash && currentHash !== journal.oldManifestHash) {
		throw new Error("DATA_CONFLICT: v0.4 project changed after migration; rollback would be lossy");
	}
