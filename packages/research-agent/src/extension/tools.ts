...
const KnowledgeParameters = Type.Union([
	Type.Object(
		{
...
const MonitorParameters = Type.Union([
	Type.Object(
		{
...
const ManuscriptParameters = Type.Union([
	Type.Object(
		{
...
function jsonResult<Value>(result: ResearchResult<Value>): ResearchResult<JsonValue> {
...
async function requestedManuscriptInputs(
	projectRoot: string,
...
async function requestedReviewInputs(
	projectRoot: string,
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			try {
				...
				const executionSignal = signal ?? ctx.signal ?? new AbortController().signal;
				return toolResponse(
					await trackedTool(project, ctx, options, "research.knowledge", (operation) =>
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			try {
				...
				const executionSignal = signal ?? ctx.signal ?? new AbortController().signal;
				return toolResponse(
					await trackedTool(project, ctx, options, "research.monitor", (operation) =>
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			try {
				...
				if (
					...
					const blocked = requireModelEgress(
						project,
						ctx,
						["qualitative_material"],
