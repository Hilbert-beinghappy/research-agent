...
import { hashCanonicalJson } from "./integrity.ts";
import {
...
function adapterRegistrationIssues(record: AdapterRegistrationRecord): ContractIssue[] {
	const issues: ContractIssue[] = [];
	const { conformance, manifest } = record;
	if (
		conformance.packageId !== manifest.packageId ||
		conformance.packageVersion !== manifest.packageVersion ||
		conformance.packageHash.value !== manifest.packageHash.value ||
		conformance.manifestHash.value !== hashCanonicalJson(manifest).value ||
		conformance.adapterId !== manifest.adapterId ||
		conformance.adapterKind !== manifest.adapterKind
	) {
		issues.push(
			issue(
				"conformance",
				"adapter_registration.conformance_mismatch",
				"Adapter conformance identity and hashes must match the registered manifest",
			),
		);
	}
	if (!conformance.passed || conformance.checks.some(({ passed }) => !passed)) {
		issues.push(
			issue(
				"conformance",
				"adapter_registration.conformance_failed",
				"Registered Adapter conformance must pass every check",
			),
		);
	}
	if (!manifest.isolationProfiles.includes(record.isolationProfile)) {
		issues.push(
			issue(
				"isolationProfile",
				"adapter_registration.isolation_undeclared",
				"Registered isolation profile must be declared by the Adapter package",
			),
		);
	}
	if (
		(record.status === "active" && record.isolationProfile === "legacy_trusted") ||
		(record.status === "legacy_trusted" && record.isolationProfile !== "legacy_trusted")
	) {
		issues.push(
			issue(
				"status",
				"adapter_registration.status_isolation_mismatch",
				"Adapter registration status and isolation profile are inconsistent",
			),
		);
	}
	return issues;
...
function invariantIssues(record: PersistedRecord): ContractIssue[] {
	switch (record.kind) {
		...
		case "adapter_registration":
			return adapterRegistrationIssues(record);
